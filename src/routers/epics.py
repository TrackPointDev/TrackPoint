...
class EpicManager:
    ...
    @router.post("/add")
    async def create_epic(self, epic: Epic):
        """
        ...
        try:
            self.db.create_epic(epic.model_dump(mode='json'))
    ...
    @router.get("/get")
    async def get_epics(self, epicID: Annotated[str | None, Header()] = None):
        """
    ...
    @router.put("/update")
    async def update_epic(self, epicID: Annotated[str | None, Header()], epic: Epic):
        """
        ...
        try:
            self.db.update_epic(epicID, epic)
        except Exception as e:
            raise HTTPException(status_code=500, detail=f"An error occurred: {e}")
        return {"status": 200, "message": f"Epic with title '{epicID}' updated successfully."}

    @router.delete("/delete")
    async def delete_epic(self, epicID: Annotated[str | None, Header()]):
        """
